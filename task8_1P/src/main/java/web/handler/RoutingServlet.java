...
public class RoutingServlet {
    ...
    public RedirectView q1(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        RedirectView redirectView;
        if (isCorrectAnswer(calculatedResult, resultUser)) {
    ...
    public RedirectView q3(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (isCorrectAnswer(calculatedResult, resultUser)) {
            ...
            redirectAttributes.addFlashAttribute("message", "Game Completed!");
        } else {
            redirectView = new RedirectView("/q3", true);
            redirectAttributes.addFlashAttribute("message", "Wrong answer or invalid input, try again.");
        }
    ...
    private boolean isCorrectAnswer(double calculatedResult, String resultUser) {
        if (Double.isNaN(calculatedResult) || resultUser == null || resultUser.trim().isEmpty()) {
            return false;
        }

        try {
            double userAnswer = Double.valueOf(resultUser.trim());
            return Double.compare(calculatedResult, userAnswer) == 0;
